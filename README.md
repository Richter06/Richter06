<!-- ========================= HERO ========================= -->
<h1 align="center">Olá! 👋 Eu sou o Richard R. Araújo</h1>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=24&duration=3000&pause=900&color=00F7FF&center=true&vCenter=true&width=760&lines=Front-End+Developer;Web+Developer;Estudante+de+ADS;UI%2FUX+%26+Design;Construindo+projetos+reais+e+evoluindo+todos+os+dias" alt="Typing SVG" />
</p>

<p align="center"><b>💻 Front-End</b> • <b>⚙️ Web & APIs</b> • <b>🎨 UI/UX</b> • <b>🗄️ SQL</b> • <b>🤖 IA no desenvolvimento</b></p>

---

## 👨‍💻 Sobre mim

Sou estudante de **Análise e Desenvolvimento de Sistemas** e desenvolvedor em início de carreira, com foco em **Front-End e desenvolvimento web**.

Gosto de transformar ideias em interfaces que realmente funcionam — de experiências visuais e landing pages a aplicações com **JavaScript, APIs, autenticação, banco de dados e back-end em Node.js/Express**.

Meu diferencial é unir desenvolvimento com **design**, buscando criar interfaces bonitas, responsivas e fáceis de usar. Também tenho base em **SQL, MySQL e PostgreSQL**, Git/GitHub e utilizo **IA diariamente** como apoio ao desenvolvimento, pesquisa, debugging e produtividade.

🎯 **Objetivo:** conquistar uma oportunidade de estágio em desenvolvimento web e continuar evoluindo dentro de uma equipe de tecnologia.

🌎 **Inglês avançado** — documentação técnica, estudos e comunicação.

---

## 🧰 Stack & ferramentas

### 🎨 Front-End
<p align="left"><img src="https://skillicons.dev/icons?i=html,css,js,figma" alt="HTML CSS JavaScript Figma" /></p>

### ⚙️ Back-End & APIs
<p align="left"><img src="https://skillicons.dev/icons?i=nodejs,express,python,fastapi" alt="Node.js Express Python FastAPI" /></p>

### 🗄️ Dados
<p align="left"><img src="https://skillicons.dev/icons?i=mysql,postgresql,supabase" alt="MySQL PostgreSQL Supabase" /></p>

### 🛠️ Ferramentas
<p align="left"><img src="https://skillicons.dev/icons?i=git,github,vscode,vercel" alt="Git GitHub VS Code Vercel" /></p>

---

# 🚀 Projetos em destaque

> **5 projetos que melhor representam minha evolução, minhas habilidades e a variedade do que consigo construir.**

### 🥇 01 — [Little Bee • Arte & Pintura](https://github.com/Richter06/paginaDeArtesVisuais)

**Full-Stack Web Application**

Projeto mais completo do meu portfólio atual. Uma plataforma para apresentação e gerenciamento de obras de arte, construída para uma necessidade real e evoluída de uma landing page para uma aplicação com **backend, autenticação, CRUD, banco de dados, armazenamento de imagens e painel administrativo**.

**O que demonstra:** arquitetura web • APIs • autenticação • gerenciamento de dados • upload de arquivos • integração com serviços externos • segurança básica • Front-End responsivo

<p><img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" /><img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" /><img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" /><img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=node.js&logoColor=white" /><img src="https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white" /><img src="https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white" /><img src="https://img.shields.io/badge/REST%20API-00A98F?style=flat-square&logo=fastapi&logoColor=white" /></p>

---

### 🥈 02 — [Clínica Vita+](https://github.com/Richter06/clinica-vita)

**Sistema de gestão para clínica**

Aplicação focada em **lógica de negócio e gerenciamento de dados**, com módulos para pacientes, médicos, consultas, prontuários, histórico, sinais vitais, dashboard, métricas, gráficos e geração de relatórios.

**O que demonstra:** CRUD • estados e fluxos de negócio • dashboards • manipulação de dados • visualização de informações • geração de documentos

<p><img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" /><img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" /><img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" /><img src="https://img.shields.io/badge/Chart.js-FF6384?style=flat-square&logo=chart.js&logoColor=white" /><img src="https://img.shields.io/badge/jsPDF-FF6F00?style=flat-square&logo=adobeacrobatreader&logoColor=white" /></p>

---

### 🥉 03 — [Portfólio pessoal](https://github.com/Richter06/portfolio)

**Front-End • UI/UX • identidade profissional**

Meu espaço de apresentação profissional, desenvolvido para transformar projetos, habilidades e identidade visual em uma experiência própria e responsiva.

**O que demonstra:** composição visual • responsividade • animações • interação • organização de conteúdo • atenção à experiência do usuário

<p><img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" /><img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" /><img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" /><img src="https://img.shields.io/badge/UI%2FUX-7B61FF?style=flat-square&logo=figma&logoColor=white" /></p>

---

### 🏛️ 04 — [Mitologias](https://github.com/Richter06/Mitologias)

**Experiência interativa • Front-End criativo**

Projeto visual dedicado às mitologias grega, nórdica e egípcia, com navegação temática, animações e uma forte preocupação com ambientação e storytelling visual.

**O que demonstra:** JavaScript • DOM • animações • responsividade • design de interfaces • experiência do usuário

<p><img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" /><img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" /><img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" /></p>

---

### 🎨 05 — [Landing Page Artística](https://github.com/Richter06/landingPageArt)

**UI/UX • identidade visual • aplicação real**

Landing page desenvolvida para uma artista, unindo apresentação de obras, identidade visual, responsividade, animações e recursos de compartilhamento social.

**O que demonstra:** design aplicado • CSS avançado • responsividade • SEO/Open Graph • interação • atenção aos detalhes

<p><img src="https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white" /><img src="https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white" /><img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" /><img src="https://img.shields.io/badge/SEO%20%2F%20Open%20Graph-4285F4?style=flat-square&logo=google&logoColor=white" /></p>

---

## 💡 O que estou construindo na prática

```text
🎨 Interfaces & UI/UX       ████████████████████  Front-End
⚡ JavaScript & Web         ███████████████████░  Interações
⚙️ APIs & Back-End          ████████████████░░░░  Node / Express
🗄️ Dados & CRUD             ███████████████░░░░░  SQL / Supabase
🚀 Projetos reais           ████████████████████  Do conceito ao deploy
```

> Os indicadores representam **áreas de foco e experiência prática**, não porcentagens de domínio.

---

## 📊 GitHub

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Richter06&theme=tokyonight" alt="Resumo do perfil GitHub" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=Richter06&theme=tokyonight" alt="Estatísticas do GitHub" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Richter06&theme=tokyonight" alt="Linguagens mais usadas nos commits" />
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Richter06&label=Visualizações%20do%20perfil&color=0e75b6&style=flat" alt="Contador de visualizações do perfil" />
</p>

---

## 🌐 Vamos conversar?

<p align="center">
  <a href="https://www.linkedin.com/in/richard-r-ara%C3%BAjo/"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
</p>

<p align="center"><i>Aberto a oportunidades de estágio e a projetos que me permitam aprender, construir e contribuir.</i></p>

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,100:2c5364&height=120&section=footer" alt="Footer" />
</p>
